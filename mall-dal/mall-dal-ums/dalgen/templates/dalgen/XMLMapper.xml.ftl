...
<#--生成BaseResultMap-->
    <#if xmlMapper.table.columnList?size gt 0>
        <resultMap id="BaseResultMap"
                   type="${xmlMapper.doClass.packageName}.${xmlMapper.doClass.className}">
            <#list xmlMapper.table.columnList as column>
                <#if column.sqlName?upper_case =="ID">
                    <id column="${column.sqlName}" property="${column.javaName}"
                        jdbcType="${column.sqlType}"
                        javaType="${fun.javaTypeTrun(column.javaType)}" ${fun.typeHandler(column)}/><#else>
                    <result column="${column.sqlName}" property="${column.javaName}"
                            jdbcType="${column.sqlType}"
                            javaType="${fun.javaTypeTrun(column.javaType)}" ${fun.typeHandler(column)}/></#if>
            </#list>
        </resultMap>
    </#if>
...
<#--生成自定义ResultMap-->
    <#list xmlMapper.resultMaps as resultMap>
        ...
        <resultMap id="${resultMap.name}"
                   type="${resultMap.packageName}.${resultMap.className}" ${extends}>
            <#list resultMap.columnList as column>
                <#if column.sqlName?upper_case =="ID">
                    <id column="${column.sqlName}" property="${column.javaName}"
                        javaType="${fun.javaTypeTrun(column.javaType)}"/><#else>
                    <result column="${column.sqlName}" property="${column.javaName}"
                            javaType="${fun.javaTypeTrun(column.javaType)}"  ${fun.typeHandler(column)}/></#if>
            </#list>
            <#list resultMap.associations as association>
                <association property="${association.property}" column="${association.column}"
                             select="${lib.genDOMapperNameSpace(dalgen.doMappers,association.tableName)}.${association.methodName}"/>
            </#list>
            <#list resultMap.collections as collection>
                <collection property="${collection.property}" column="${collection.column}"
                            select="${lib.genDOMapperNameSpace(dalgen.doMappers,collection.tableName)}.${collection.methodName}"/>
            </#list>
        </resultMap>
    </#list>
